...
pacman::p_load(tidyverse, forcats, pander, stringr, jsonlite, lubridate, jrfTools, extrafont, RcppRoll, ggrepel, DT)
```
...
```{r groupme-example, fig.cap = 'Screenshot showing posts, mentions, and likes', out.width = '40%'}
...
```{r posts-data-frame, cap.fig = "test me"}
posts <-
  posts_per_user %>%
  group_by(posts) %>%
  summarise(users = n()) %>%
