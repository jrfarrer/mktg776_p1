...
pacman::p_load(tidyverse, forcats, pander, stringr, jsonlite, lubridate, jrfTools, extrafont, RcppRoll, ggrepel)
```
...
There are **`r nrow(users)`** users in the Wharton 2018 GroupMe. There are approximately 850 members of the Wharton 2018 MBA class. Though the group was created in January 2016, we trimmed the dataset to August 8, 2016 (first day of pre-term) to provide an accurate window in which to observe the actions of the users. In other words, all users have the same observation period. We removed users from the dataset that have left the group and discuss the possibility of late joiners in the final Limitations section. The last post in our dataset is **`r msgs %>% arrange(desc(created_at)) %>% select(created_at) %>% head(1) %>% mutate(created_at = as.character(created_at)) %>% unlist()`**. There have been **`r nrow(msgs)`** posts by **`r msgs %>% summarise(n_distinct(user_id)) %>% unlist()`** distinct users. Below is a time series of the posts:
...
We expect to observe difference in heterogeneity of each of the three events. For example, we would presume that there is more heterogeneity in **like rate** than in **post rate** as liking is less visible and risky (to one's reputation) than posting in a group of `r nrow(users)`.
...
Below is a table that shows the estimated number of users for posts counts less than five by the three parameter estimation techniques. A plot showing all post counts follows. We see that the methods are not that different, but method of moments certainly performs the worst. 
...
In order to perform the $\chi$^2 goodness of fit test for the NBD model, we need rollup the tail so that 80% of the expected counts have more than 5 counts. We create a `r n_posts`+ bucket and calculate the $\chi^2$ test statistic and $p$-value for each paremeter estimation method using `r n_posts` - 2 - 1 = `r n_posts - 2 - 1` degrees of freedom. Based on the $p$-values shown below, we have no evidence that the data came from the NBD model. Nevertheless, the plot above shows a relatively good fit, at least for the estimates from MLE and means and zeros.
...
mentions_gof_test <- 
  mentions_expected %>%
      filter(model != "MLE (Zero-Inflated)") %>%
      mutate(posts = if_else(mentions >= n_mentions, paste0(n_mentions,"+"), as.character(mentions))) %>%
      group_by(model, mentions) %>%
      summarise(
        Actual = sum(Actual)
        , expected = sum(expected)
...
We follow the same process for likes given as we did for posts and mentions. We note that the tail is a bit longer for likes as some users do lot of post-liking. The median number of likes given is **`r median(likes_per_user$likes)`** likes though the mean is **`r mean(likes_per_user$likes)`** likes.
...
likes_gof_test <- 
  likes_expected %>%
      filter(model != "MLE (Zero-Inflated)") %>%
      mutate(posts = if_else(likes >= n_likes, paste0(n_likes,"+"), as.character(likes))) %>%
      group_by(model, likes) %>%
      summarise(
        Actual = sum(Actual)
        , expected = sum(expected)
...
Finally we perform the $\chi^2$ goodness of fit test and first rollup the tail so that 80% of the expected counts have more than 5 counts. We create a `r n_likes`+ bucket and calculate the $\chi^2$ test statistic and $p$-value for each paremeter estimation method using `r n_likes` - 2 - 1 = `r n_likes - 2 - 1` degrees of freedom. Based on the $p$-values shown below, we have no evidence that the data came from the NBD model. Moreover, we note how large the $\chi^2$ test statistics are for the likes given in comparison to posts or mentions, indicating how poor a fit the NBD model is for the likes given data.
...
Lp %>% 
  ...
  geom_point(data = rule_8020, aes(wx = p, y = L_p, colour = variable)) + 
  geom_label_repel(data = rule_8020, aes(wx = p, y = L_p, colour = variable, 
...
for (i in 1:nrow(gender_combinations)) {
...
gender_params2 <-
  gender_params %>%
    ...
    arrange(activity, gender, model)
```

```{r}
gender_params2 %>%
  pander(caption = "", missing = "", round = 4)
```
...
gender_expected %>%
  ggplot() +
    ...
    labs(x = NULL, y = "Users", title = "NBD Models by Activity by Gender", colour = NULL) +
...
Lp_gender %>% 
  ...
  scale_x_continuous(breaks = scales::pretty_breaks(), labels = scales::percent) + 
  scale_y_continuous(labels = scales::percent) +
  geom_point(data = rule_8020_gender, aes(wx = p, y = L_p, colour = gender)) + 
  geom_label_repel(data = rule_8020_gender, aes(wx = p, y = L_p, colour = gender, 
                    label = scales::percent(L_p)), show.legend = FALSE,
                    family = "DecimaMonoPro") +
  theme_jrf(users_v = "rstudio") + 
  theme(legend.position	= "top") + 
